...
前言及注意：使用的话需要升级React相关的库的版本(React、react-dom等)，只能在函数里面使用，不可在class里面使用，将原本class的一些周期函数封装成API，可以用HOOK取代class，但是官网也建议先不要替换，还有部分暂时无法替代的，比如暂时还没有对应不常用的`getSnapshotBeforeUpdate` 和` componentDidCatch` 生命周期的 Hook 等价写法,目前 Hook 还处于早期阶段，一些第三方的库可能还暂时无法兼容 Hook。不要在循环，条件或嵌套函数中调用 Hook（因为React 靠的是 Hook 调用的顺序来关联内部的state）

> API

`useState`：类似于class的state，返回state和setState，函数组件可以用多个useState，
```
const [name, setName] = useState('陈老')
const [hobby, setHobby] = useState(['basketball', 'swimming'])
```

`useEffect`：等同于class的`componentDidMount`和`componentDidUpdate`，实现了在首次挂载和每次更新都会执行(在class没有这样的周期函数)，第一个参数是一个函数，可以通过第二个参数（数组）来提高性能，避免任何更新都在渲染，只要数组里面的值欲上一次更新的值不相等才会更新，相等则不执行函数，同时也可以通过传入一个空数组来使得该函数只在第一次挂载和最后卸载的时候执行。
```
解决的痛点：
1、以前我们要使得首次挂载和更新都执行某个操作需要在componetDidMount和componentDidUpdate里面写上相同的操作
componentDidMount() {
...
现在我们可以直接通过useEffect来实现(因为他是在首次挂载和每次更新都会执行的api)
2、以前我们监听某个操作在卸载的时候需要去清除，避免内存泄漏
fun () {}
componentDidMount() {
...
现在我们可以直接通过useEffect来实现，每个effect都可以返回一个函数，当在移除组件的时候会执行这个返回的函数
useEffect(() => {
  document.addEventListener('click', fun, false)
  return () => {
    document.removeEventListener('click', fun, false)
...
```
所以可见，effect使我们可以对一些同样的操作集中起来一起出来，实现了操作的完整性
